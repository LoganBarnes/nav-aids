...
namespace ltb::ils
{
    namespace
    {
        ...
        struct IlsPatternDisplayUniforms
        {
            glm::vec4 color = glm::vec4(1.0F);
            float32 line_width = 1.0F;
            WaveForm wave_form = WaveForm::Combined;
        };

        auto to_wave_form(IlsPatternForm const wave_form) -> WaveForm
        {
            switch (wave_form)
            {
            case IlsPatternForm::Sbo:
                return WaveForm::Sbo;
            case IlsPatternForm::Csb:
                return WaveForm::Csb;
            case IlsPatternForm::Carrier:
                return WaveForm::Carrier;
            case IlsPatternForm::Combined:
            case IlsPatternForm::All:
                break;
            }
        ...
        auto draw_wave(
            ...
            IlsPatternDataUniforms const& data_uniforms,
            IlsPatternDisplayUniforms const& display_uniforms
        ) -> void
        {
    ...
    IlsPatternDisplayPipeline::IlsPatternDisplayPipeline(
    ...
    auto IlsPatternDisplayPipeline::initialize(IlsPatternPipelineSettings const& settings)
        -> utils::Result<void>
    {
        ...
        auto uniform_push_constants = std::vector{
            ...
            .setOffset(0U)
            .setSize(sizeof(IlsPatternDataUniforms)),
            vk::PushConstantRange{}
            .setStageFlags(vk::ShaderStageFlagBits::eTessellationEvaluation)
            .setOffset(display_uniform_offset)
            .setSize(sizeof(IlsPatternDisplayUniforms)),
        };
    ...
    auto IlsPatternDisplayPipeline::is_initialized() const -> bool
    {
        return initialized_;
    }

    auto IlsPatternDisplayPipeline::get_data() const -> IlsPatternData
    {
        return wave_data_;
    }

    auto IlsPatternDisplayPipeline::set_data(IlsPatternData const& data) -> void
    {
        wave_data_ = std::move(data);
    }

    auto IlsPatternDisplayPipeline::draw(vlk::objs::FrameInfo const& frame) -> utils::Result<void>
    {
        pipeline_.bind(frame.command_buffer);

        LTB_CHECK(pipeline_.bind_descriptor_sets( frame ));

        auto display_uniforms = IlsPatternDisplayUniforms{
            .line_width = wave_data_.line_width,
            .wave_form = to_wave_form(wave_data_.wave_form),
        };

        auto data_uniforms = IlsPatternDataUniforms{
        ...
        // Combined wave
        if (wave_data_.wave_form != IlsPatternForm::All)
        {
